...
namespace PAI
{
    public class BinaryDecisionTree
    {
        ...
        private static BinaryDecisionTree TreeBuilder(Formula f,
        IEnumerable<Variable> variables, int varIndex, string path)
        {
            if (!string.IsNullOrEmpty(path))
                variables.ElementAt(varIndex - 1).Value = path[path.Length - 1]
                != '0';

            if (varIndex == variables.Count())
                return new BinaryDecisionTree(f.Evaluate() ? 1 : 0);

            return new BinaryDecisionTree(varIndex,
